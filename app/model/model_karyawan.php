...
class PeopleAuth {
    ...
    public function create($username, $email, $nama, $password, $repassword, $role){
        ...
        if($cekselect['id_akun'] > 0){
            ...
            if($data != null){
                if($data){
                    echo "<script>document.location.href = '../ui/header.php?aksi=pendaftaran-karyawan&info=berhasil'</script>";
                    die;
                }
            }else{
                echo "<script>document.location.href = '../ui/header.php?aksi=pendaftaran-karyawan&info=gagal'</script>";
                die;
            }
        }
    }
